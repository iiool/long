...
class Featured_Image_By_URL_Common {
	...
	function knawatfibu_overwrite_thumbnail_with_url( $html, $post_id, $post_image_id, $size, $attr ){
		...
		if( !empty( $image_data['img_url'] ) ){
			...
			$html = sprintf('<img src="%s" % class="%s" %s />', 
							$image=$image_url."?resize=1090%2C1500".'"'."width=".'"'."1090".'"'."height=".'"'."1500", $image_alt, $classes, $style);
		}
		return $html;
	}
		...
		if( !empty( $image_size ) && !empty( $image_size['width'] ) ){
			...
			if( function_exists( 'jetpack_photon_url' ) ) {
				if( isset( $image_size['crop'] ) && $image_size['crop'] == 1 ){
					return jetpack_photon_url( $image_url, array( 'fit' => "290,290" ) ).'"'."height=".'"'."290".'"';
				}else{
					return jetpack_photon_url( $image_url, array( 'resize' => "1090,1500" ) ).'"'."width=".'"'."1090".'"'."height=".'"'."1500".'"';
				}
			}
