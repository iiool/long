...
function blockpress_theme_conditional_assets($html, $block)
{
	...
	if(!is_admin()){//prevent loading conditional assets in admin
		...
		if (isset($block['blockName'])) {
			if (!empty($block['attrs']['className'])) {
				if ($block['blockName'] == 'core/button') {
					...
					if (str_contains($block['attrs']['className'], 'fr-tablet-collapse') !== false) {
						$block_style .= '@media only screen and (max-width: 992px){.fr-tablet-collapse{display:block !important}.fr-tablet-collapse.wp-block-columns:not(.is-not-stacked-on-mobile)>.wp-block-column:not(:first-child){margin-left:0}}';
					}
			...
			if($block['blockName'] == 'blockpress/lottie'){
				$block_style .='.fr-lottieloader img{transition: opacity 1s ease-in}.fr-lottieloader lottie-player:empty{display:none}.fr-lottieloader.lottie-loaded lottie-player{display:inline-block}.fr-lottieloader.lottie-loaded img{opacity:0}';
				if(!empty($block['attrs']['widthMobile'])){
					$block_style .='@media(max-width:768px){.'.$block['attrs']['id'].' img, .'.$block['attrs']['id'].'  lottie-player{width: '.$block['attrs']['widthMobile'].'px!important}}';
				}
				if(!empty($block['attrs']['heightMobile'])){
					$block_style .='@media(max-width:768px){.'.$block['attrs']['id'].' img, .'.$block['attrs']['id'].'  lottie-player{height: '.$block['attrs']['heightMobile'].'px!important}}';
				}
				wp_enqueue_script('blockpress-lottieloader');
			}
		}
